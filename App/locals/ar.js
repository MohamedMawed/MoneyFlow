export default {
    Arabic:'عربي',
    English:'انجليزي',
    Bills:'الفواتير',
    Groceries:'البقالة',
    Transportation:'المواصلات',
    Shopping:'التسوق',
    Education:'الدراسة',
    HealthFitness:'الصحة واللياقة',
    Entertainment:'الترفيه',
    GiftsDonations:'الهدايا والتبرعات',
    Business:'عمل تجاري',
    Salary:'المرتب',
    ok:'موافق',
    send:'ارسال',
    enterValidEmail:'من فضلك ادخل البريد الالكترونى صحيح',
    enterYourEmail:'من فضلك ادخل البريد الالكترونى',
    PleaseSpecifyTheValue:" يرجى ادخال المبلغ",
    PleaseSpecifyTheDate:"يرجى ادخال التاريخ",
    PleaseSelectedIcon:"يرجى تحديد الايقونه",
    PleaseCheckYourEmail:'تم ارسال رابط اعادة تعين الرقم السرى',

    ResetPassword:'اعادة تعين الرقم السرى',
    Remaining: 'متبقي',
    howMuch:'كم المبلغ ؟',
    days: 'يوم',
    Edit: 'تعديل',
    titlePlan: 'ابدأ الادخار الأن لأهدافك',
    appName: 'فلوسي',
    weekly: 'أسبوعية',
    Monthly: 'شهرية',
    annual: 'سنوية',
    logout: 'تسجيل الخروج',
    noGols:'لا يوجد أهداف حتى الأن',
    titleBudget: 'حدد ميزانياتك لتنظم أمورك المالية',
    noMonthlyBudgets: 'لا يوجد ميزانيات شهرية حتى الان',
    noWeeklyBudgets: 'لا يوجد ميزانيات اسبوعية حتى الان',
    noWeeklyAnnual:'لا يوجد ميزانيات سنوية حتى الان',
    editBudget: 'تعديل الميزانية',
    tap: {
        Plan: 'الأهداف',
        Home: 'الرئيسية',
        Report: 'الميزانيات',
        Setting: 'الاعدادات'
    },
    TitleIntroScreen: ['تتبع نفقاتك ودخلك', 'حدد ميزانيتك الشهرية', 'حدد أهدافك المالية'],
    ContentIntroScreen: ['سنساعدك على معرفة أين يذهب مالك وكيف تحافظ على دخلك', 'تحديد ميزانية شهرية سيساعدك كثيرا على التوفير لادخار المزيد من المال',
        'هل تخطط لشراء شىء ؟ \n تستطيع تحديد هدف شراء منزل مثلا بموعد محدد وتتبع التقدم'],
    Register: 'تسجيل حساب',
    Login: 'دخول',
    Next: 'التالي',
    ChooseIcon: 'اختر أيقونة مناسبة',
    forgetPassword: 'نسيت كلمة المرور',
    fbLogin: 'الدخول بواسطة فيسبوك',
    fbRegister: 'تسجيل حساب بواسطة فيسبوك',
    googleLogin: 'الدخول بواسطة جوجل',
    googleRegister: 'تسجيل حساب بواسطة جوجل',
    email: 'البريد الإلكتروني',
    password: 'كلمة المرور',
    name: 'الاسم',
    current_password : 'كلمة السر الحالية',

    monthlyReport: 'التقارير الشهرية',
    Income: 'الدخل',
    Expenses: 'المصروفات',
    inWallet: 'في محفظتك',
    currencyLE: 'جنيه',

    headingIcomeScreen: 'اضافة دخل أو مصروف',
    description: 'لكي يتم احتسابهم',
    save: 'حفظ',
    noPlans: 'لا يوجد اهداف',
    newPlan: 'هدف جديد',
    newBudget: 'ميزانية جديدة',
    chooseIcon: 'اختر أيقونة مناسبة',
    success: 'نجاح',
    budgets: 'الميزانيات',
    chooseCat: 'اختر التصنيف',
    budget: 'ميزانية',
    startDate: 'تاريخ البدأ',
    endDate: 'تاريخ الانتهاء',
    noBudgets: 'لا يوجد ميزانيات',
    plans: 'الأهداف',
    incomeAdded: 'تم إضافة الدخل بنجاح',
    validNumber: 'برجاء ادخال رقم صحيح',
    alertChooseType: 'برجاء إختيار التصنيف',

    accountset: ' اعدادات الحساب',
    changeLang: 'تغيير اللغة',
    changeCur: 'تغيير العملة',
    exportImport: 'استخراج البيانات وإدخالها',




    addPlan_placeHolder_target: 'الهدف',
    addPlan_placeHolder_startWith: 'مبلغ البداية',




    allPlans_remain: 'يوم متبقي',
    allPlans_add: 'إضافة',
    allPlans_edit: 'تعديل',
    allPlans_remove: 'حذف',



    addPlan_nameErr: 'إسم الهدف إلزامي',
    addPlan_targetErr: 'تكلفة الهدف إلزامي',
    addPlan_endDateErr: 'تاريخ إنتهاء الهدف إلزامي',
    addPlan_iconErr: 'شعار الهدف إلزامي',


    addMoneyPopUp_placeHolder: 'المبلغ',


    edit_goal_title: 'تعديل الهدف',
    add_money_title: 'إضافة مبلغ',

    edit_profile_title: 'تعديل الملف الشخصي',
    edit_profile_new_password: 'كلمة السر الجديدة',

    edit_profile_requiredText: 'لتغيير البريد الإلكتروني أو كلمة السر يجب إدخال كلمة السر الحاليى',


    home_no_stat: 'لا يوجد تقارير',
    addTypeName:"الاسم",
    addTypeHeader:"اضافة تصنيف",
    addTypeChooseIcon:"اختر أيقونة"
}